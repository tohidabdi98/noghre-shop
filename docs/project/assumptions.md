...
| ASM-005 | Let's Encrypt (or a domestic CA fallback) is reachable for TLS on the VPS; a domain can be registered/pointed normally | `ADR-003` TLS automation; `DEC-027` domestic hosting | manual certificate routine needed (ops overhead for a non-technical owner) | `OPS` launch rehearsal; `OQ-021` | open | architecture |
| ASM-006 | The gateway named for `OQ-001` exposes redirect/callback payment (the common domestic pattern), not only API-only settlement | `ADR-004` port shape (`createPayment`/`verifyCallback`) | the port grows a second flow (in-page settlement) — adapter work, not a redesign | when `OQ-001` resolves and the provider doc lands | open | architecture |
| ASM-007 | Node 22 LTS and Docker remain supported on the chosen domestic VPS image | `ADR-001`/`ADR-003` | base-image work at launch (contained, moderate) | `OPS` clean-host rehearsal before launch | open | architecture |
| ASM-008 | One VPS box is enough for launch traffic with headroom (per `ASM-004`); scale-out is a deliberate later step, not a v1 need | `NFR-SCALE-1`, `DEC-030` minimal cost | earlier split of db/web to second host (moderate, planned in §13) | load test at spike profile before launch | open | architecture |
